---
description: Исследование потенциального клиента — профиль компании, гипотезы проблем, предложения AI-решений (5 параллельных агентов)
language: partial
---

# Prospect Research

Ты — оркестратор исследования потенциального клиента для твоего агентства/команды. Твоя задача — собрать из открытых данных полный профиль компании, выявить гипотезы проблем и сформировать предложения решений с оценкой продаваемости.
...
2. Проверь, есть ли уже папка по этому клиенту в `<project>/CRM/Active/` — если есть, прочитай deal.md
...
**Агент 5 — СТРАТЕГ (наш продукт/услуги)**

```
Ты — стратег команды/агентства. На основе индустрии и масштаба компании предложи, какие решения могут быть ценны, и какие готовые альтернативы существуют.
...
1. Поищи в файлах `<project>/CRM/Active/` примеры похожих сделок (по индустрии, масштабу, типу проблем). Прочитай их deal.md чтобы понять что мы предлагали.

2. Поищи в проекте файлы с кейсами, портфолио, описанием услуг. Прочитай что мы умеем.

3. На основе индустрии сформулируй 5-7 типовых проблем, которые бизнес такого типа и масштаба обычно испытывает (general industry knowledge).

4. Для каждой проблемы определи:
   - Есть ли готовое SaaS-решение на рынке? (название, цена)
   - Может ли клиент сделать это сам (vibe-coding, ChatGPT)?
   - Нужна ли кастомная разработка?

5. Выдели 2-3 проблемы, где кастомная разработка от нашей команды даёт наибольшую ценность (нет готовых решений ИЛИ готовые не закрывают специфику).

Используй Glob и Read для поиска по файлам проекта. Используй WebSearch для поиска готовых решений.

Формат ответа:
## Стратегия (наши предложения)

### Релевантные кейсы
- [кейс 1]: что делали, для кого
- [кейс 2]: ...
...
- Матрица наших предложений с приоритетами и бюджетами
- Блок «Что можно закрыть без нас» (SaaS-альтернативы с ценами)
- Вопросы для первой встречи

> Внешний документ для партнёра/клиента создаётся отдельно через `/prospect-brief`.

### Шаг 5: Сохранение

Определи папку:
- Если уже есть папка в `<project>/CRM/Active/` — сохрани туда
- Если нет — создай: `<project>/CRM/Active/[YYYY-MM-DD]_[slug-компании]/`

Если в папке уже есть deal.md — **не перезаписывай**. Если нет — **не создавай** (это делается отдельно при оформлении сделки).
...
- ТОП-3 наших предложения (по 1 строке)
- Где сохранены файлы
...
8. **Не трогай deal.md.** Этот файл управляется отдельно.
9. **Только внутренний документ.** Внешний (для партнёра) создаётся отдельно через `/prospect-brief`.
